...
const Transactions = () => {
  ...
  const getIcon = (tx) => {
    ...
    if (txType === "TRANSFER" && tx.description === "Transfer Keluar") return ArrowUp;
    if (txType === "TRANSFER" && tx.description === "Transfer Masuk") return ArrowDown;

    const combined = `${tx.category} ${tx.subcategory} ${tx.description}`.toLowerCase();
    if (combined.includes("makan") || combined.includes("minum")) return Utensils;
  ...
  const normalized = transactions
    ...
    .filter((tx) => {
      const matchesSearch = [tx.label, tx.category, tx.wallet_name, String(tx.amount)]
        .join(" ")
        .toLowerCase()
        ...
        activeDate === "all"
          ...
          : activeDate === "today"
            ...
            : activeDate === "yesterday"
              ...
              : activeDate === "month"
                ? transactionDate
                  ? daysBetween(transactionDate, today) >= 0 && daysBetween(transactionDate, today) < 30
                  : false
                : true;
            ...
            <section key={day} className="finance-card p-6 md:p-8">
              ...
              <div className="mt-6 space-y-4">
                {txs.map((tx) => (
                  <div
                    key={tx.id}
                    className="group flex items-start justify-between gap-4 rounded-[22px] bg-[#FAFCF7] px-4 py-4 transition hover:bg-[#F3F8EE] dark:bg-[#253044] dark:hover:bg-[#2D3A52] md:items-center"
                  >
                    <div className="flex min-w-0 items-center gap-4">
                      <div className={cn("flex h-12 w-12 shrink-0 items-center justify-center rounded-[18px]", getBg(tx))}>
                        <tx.icon className="h-5 w-5 shrink-0" />
                      </div>
                    ...
                    <div className="flex shrink-0 items-center gap-3">
                      <p className={cn("text-sm font-bold md:text-base", getColor(tx))}>
